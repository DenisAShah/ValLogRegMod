...
label(cdata$ter_pos) <- "Primary tumor teratoma positive "
label(cdata$preafp) <- "Elevated prechemotherapy AFP"
label(cdata$prehcg) <- "Elevated Prechemotherapy HCG"
...
res_weak_cal <- cbind(
  ...
  "Lower .95" = slope_CI[2,1],
  "Upper .95" = slope_CI[2,2]
)
...
<details>
  <summary>Click to expand code</summary>
```{r, moder_cal, fig.align='center',echo=TRUE, eval=FALSE, warning=FALSE, message=FALSE, fig.width=6, fig.width=6}
# Models
dd <- rms::datadist(rdata, adjto.cat = "first")
options(datadist = "dd")
fit_lrm <- rms::lrm(tum_res ~
...
```{r, moder_cal, fig.align='center', warning=FALSE, eval=TRUE, fig.height=6,fig.width=6}
```
...
More flexible, nonlinear, calibration curve can be considered using the 'secondary' logistic regression using splines transformations of the predicted probabilities. Here we provide an example using a five-knot restricted cubic splines using ```rms::rcs()```.  

<details>
  <summary>Click to expand code</summary>
```{r, moder_flex_cal, fig.align='center',echo=TRUE, eval=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=6}
# Models
dd <- rms::datadist(rdata, adjto.cat = "first")
options(datadist = "dd")
fit_lrm <- rms::lrm(tum_res ~
...
```{r, moder_flex_cal, fig.align='center', warning=FALSE, eval=TRUE, fig.height=6, fig.width=6}
```

The corresponding calibration measures ICI, E50, and E90 may be calculated based on the estimated predicted probabilities using the a more flexible 'secondary' logistic regression model. 
...
readoff_dca <- rbind.data.frame(
  dca_rdata$net.benefit[dca_rdata$net.benefit$threshold == 0.2,],
