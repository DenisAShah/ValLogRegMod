...
# C-index
c_apparent <- round(0.5 * 
                      (1 +
                      val_lrm["Dxy","index.orig"]), 2) 

# Development set (Apparent validation)
val_lrm <- validate(fit = fit_lrm, B = 500)
rdata$pred <- predict(fit_lrm, type = "fitted.ind")
...
c_optimism <- round(0.5 * 
                      (1 +
                      val_lrm["Dxy","index.corrected"]), 2)

val.prob(predict(fit_lrm, type = "fitted.ind"), as.numeric(rdata$tum_res) - 1, 
                     pl = FALSE) %>% round(3)
...
mean_group <- tapply(rdata$pred, rdata$tum_res, mean)
discr_slope <- round(abs(mean_group[1] - mean_group[2]), 2)
