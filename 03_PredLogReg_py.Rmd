...
<details>
  <summary>Click to expand code</summary>
```{python, discrimination, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE, include=FALSE}
...
def bootstrap_cv_lrm(data, y, X, B = 2000):
  ...
  for j in range(B):
    # Bootstrapping development data
    bdata[j] = sk.utils.resample(data, replace = True, n_samples = len(data)) # bootstrapping development data
...
#### Internal validation (optimism-corrected bootstrap)
y = rdata["tum_res"]
X_rdata = rdata[["ter_pos_Yes", "preafp_Yes", "prehcg_Yes", "sqpost", "reduc10"]]
X_rdata = X_rdata.assign(intercept = 1.0)
...
for j in range(B):
  ...
  # Bootstrapping development data
  bdata[j] = sk.utils.resample(rdata, 
      replace = True, 
      n_samples = len(rdata)) # bootstrapping development data
      
  bdata[j] = bdata[j].assign(intercept = 1.0)
  
  X_boot[j] =  bdata[j][["ter_pos_Yes", "preafp_Yes", 
                                  "prehcg_Yes", "sqpost", 
                                  "reduc10", "intercept"]]
  
  # Logistic regression model in every bootstrapped data
  lrm_boot[j] = smf.GLM(bdata[j].tum_res, X_boot[j], family = smf.families.Binomial())
  coeff_boot[j] = lrm_boot[j].fit().params # coefficients
  ...
  ## C-statistic --
  cstat_boot[j] = concordance_index(bdata[j].tum_res, lp_boot[j]) 
  cstat_orig[j] = concordance_index(rdata.tum_res, lp_orig[j]) 
  cstat_diff = np.subtract(cstat_boot, cstat_orig)
  cstat_opt = np.mean(cstat_diff)
  
  ## Discrimination slope --
  boot_out_group[j] = pred_boot[j].groupby(bdata[j].tum_res).mean()
  dslope_boot[j] = abs(boot_out_group[j][1] - boot_out_group[j][0])
  
  orig_out_group[j] = pred_orig[j].groupby(rdata.tum_res).mean()
  dslope_orig[j] = abs(orig_out_group[j][1] - orig_out_group[j][0])
  dslope_diff = np.subtract(dslope_boot, dslope_orig)
  dslope_opt = np.mean(dslope_diff)
...
res_discr = np.reshape(
  (cstat_rdata,
   np.percentile(cstat_dev_b, q = 2.5),
   np.percentile(cstat_dev_b, q = 97.5), 
   
   cstat_rdata - cstat_opt,
   None,
   None,
   
   cstat_vdata,
   np.percentile(cstat_val_b, q = 2.5),
   np.percentile(cstat_val_b, q = 97.5),
   
   dslope_rdata,
   np.percentile(dslope_dev_b, q = 2.5),
   np.percentile(dslope_dev_b, q = 97.5),
   
   dslope_rdata - dslope_opt,
   None,
   None,
   
   dslope_vdata,
   np.percentile(dslope_val_b, q = 2.5),
   np.percentile(dslope_val_b, q = 97.5)),
